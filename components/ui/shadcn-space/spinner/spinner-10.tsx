...
export function TextMorph({ words, interval = 0, className }: TextMorphProps) {
  ...
  ({
    ...
    return (
      <motion.div
        ref={containerRef}
        className={cn(
          "w-full rounded-b-lg overflow-hidden flex flex-col items-center justify-center bg-background-transparent",
          config.container,
          className,
        )}
        {...props}
      >
        ...
        <div className="flex items-center gap-1 text-muted-foreground select-none">
          <TextMorph
            words={words}
            interval={0}
            className={cn("font-xl", config.fontSize)}
          />
        </div>
      </motion.div>
    );
